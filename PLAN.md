...
**Status:** M0–M7 built and live at optionaut.org (2026-09-04). Parking lot next.
...
- ✅ **M5 — Sims & sharing** (2026-07-21): shareable strategy URLs (expiry/strikes/dials in query params, nearest-dte matching so links survive calendar regens, "share this setup" copy control; condor wing-symmetry rider grew into a chain fix — far wings now quote zero-bid/nickel-ask like real markets so 4-leg strategies fit every expiry); scenario sims shipped as (a) "Earnings night" tour — IV-crush replay on PULSAR flown long then short, real dial gates, and (b) position-sizing overlay — 500 seeded Monte Carlo expiries at the position's own IV vs. an account number, honest zero-drift assumption. Remaining M5 ideas (more strategies) → parking lot. (~~Alpaca proxy + caching~~ — killed by D11: redistribution prohibited at every affordable tier.)
- ✅ **M6 — The simulated market** (2026-07-21, D11): six fictional archetype securities generated by the in-house engine, Cboe capture machinery retired, bring-your-own-data (quick-build + manual Cboe CSV upload) stored localStorage-only.
- ✅ **M7 — Tours for beginners** (2026-09-04, D12): a first-time usability review of tour mode (22 findings) and the fix pass — caption card in its own lane, scene-driven steps, spotlighted controls, prediction reveals, saved progress, a first-flight offer, and a copy pass on jargon.
...
| D12 | **Tours for beginners** (2026-09-04, from a usability review flown as a first-time visitor). (a) The tour caption card lives in its own lane between the stage and the readout — never over the pills, labels, or numbers a caption points at. (b) Every step declares a *scene* (ticker, strategy, view, expiry, strikes, dials); arrivals apply the merged scene synchronously before the step renders, so gates are never judged against stale dials, back navigation shows the right instrument, a learner's detour heals at the next step, and no tour depends on the expiry the learner happened to have selected. Gates are relative to spot and expiry, never hard-coded. (c) Prediction prompts reveal the answer from the live numbers; every gate has a quiet "skip" (PLAN §4.2: interaction deepens, never required). (d) The control a step names lights up in the HUD; the target is derived from the tour, not mirrored in state. (e) Escape closes one layer at a time; progress persists per browser with resume/done in the tours sheet and a next-tour hand-off; reaching the last caption counts as done. (f) A first visit gets a "start the tour / just explore" offer; the tour chip carries a quiet static glow until a tour has been flown (motion only with meaning). (g) Terms leaked as plain text got `<Term>`s and three glossary entries (earnings, straddle, iron condor). Deliberately not done: restoring the pre-tour scene on exit (tours pin their own scene on entry instead) and a single unified keymap (the tour keeps its own arrow-key listener). | ✅ Built & deployed 2026-09-04 |
| D11 | **The simulated market** (2026-07-21, user-directed after licensing research). Trigger: user asked whether serving Cboe's delayed quotes could "get us in trouble." Research (three parallel agents over Cboe/OPRA/vendor terms): Cboe's site ToS prohibit automated extraction and republication; OPRA's delayed-data redistribution fee starts ~$1,500/month (historical next-day data is fee-exempt but vendor contracts still bind whoever fetches it); Alpaca's terms prohibit redistribution outright regardless of non-commercial intent; every keyless/hobby source (Yahoo included) is personal-use-only; marketdata.app public display requires a custom-priced commercial tier. Conclusion: **no affordable legal path to publicly serving real chains.** Decision (user-confirmed): (a) the bundled market is **fictional** — six archetype securities with ≥6-letter symbols (real US listings max at five, so collision is impossible), personalities spanning the vol spectrum (COSMOS sleepy index 14% IV → PULSAR binary-event biotech 95%), generated deterministically by our own engine (`src/lib/sim/market.ts`, seeded GBM histories walked backward to the spot, smile+term-structure IVs, chains priced by the same BS engine that powers the cockpit); (b) **real securities are bring-your-own** — a quick-build from user-typed numbers, or Cboe's quotedata CSV which users download *manually* (manual personal lookup is what Cboe's terms permit; the app never fetches). User data is cached **localStorage-only, never server-side** — persistence without republication. Retired: `scripts/capture-snapshot.mjs`, `.github/workflows/refresh-data.yml`, the nightly-data badge, and the any-symbol serverless route (parking lot) — all superseded. No inquiries/licensing outreach drafted (user-directed). | ✅ Built 2026-07-21 |

---

## 11. Parking lot

- ~~Product name~~ → **Optionaut** (D9). ~~Domain~~ → **optionaut.org** registered via Vercel 2026-07-21, live with auto-TLS; `optionaut.vercel.app` remains as a secondary alias. ~~Trademark search~~ → user checked USPTO 2026-07-21: no conflicting registration found; clear to promote.
- ~~Any-symbol search via on-demand Cboe snapshots~~ → superseded by D11: serving real chains from our infrastructure is redistribution at any freshness. Shipped instead as **bring-your-own data** (quick-build + manual CSV upload, localStorage-only). `/t/*` and `/lab/*` accept any plausible symbol so custom-market deep links render.
