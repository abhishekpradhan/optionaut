    ...
    <>
      ...
      <div className="space-y-3.5 text-[13.5px] leading-relaxed text-muted-foreground">
        ...
        <p>
          ...
          The six hangar securities — COSMOS through PULSAR — are{" "}
          <em>fictional</em>. Their histories, chains, and volatility surfaces are generated by
          our own engine to behave like six recognizable market personalities (a sleepy index
          fund, a binary-event biotech…), so nothing here is anyone&apos;s market data and
          nothing is ever stale. All interactive math is computed in your browser by our own
          Black-Scholes-Merton engine. Honest idealizations: US equity options are
          American-style while the model is European; entries assume mid fills; &quot;win
          odds&quot; assume prices wander randomly at current implied volatility.
        </p>
        <p>
          <span className="font-medium text-secondary-foreground">Want a real security?</span>{" "}
          Exchanges license the display of real quotes, so we can&apos;t publish them — but you
          are allowed to look them up yourself. Use the{" "}
          <span className="text-secondary-foreground">+ data</span>{" "}
          chip to build a security
          from numbers off your broker&apos;s screen, or upload the CSV you download manually
          from Cboe&apos;s delayed-quotes page. Either way it lives only in your browser
          {snapshot?.source === "custom"
            ? ` — like ${snapshot.symbol}, added ${fmtDateLong(snapshot.capturedAt.slice(0, 10))}.`
            : "; we never see it."}
        </p>
        <p>
